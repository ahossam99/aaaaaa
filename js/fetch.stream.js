...
function tryHLSPlayer(name, token) {
	var serverip="https://demo.stella.live";
	var port = "5443";
	var path =  serverip + ":" + port + "/WebRTCAppEE/streams/";
	fetch(path + name +"_adaptive.m3u8", {method:'HEAD'})
	.then(function(response) {
		...
		else 
		{
			...
			fetch(path + name +".m3u8", {method:'HEAD'})
			.then(function(response) {
				if (response.status == 200) {
					//m3u8 exists, play it
					...
					fetch(path + name +".mp4", {method:'HEAD'})
					.then(function(response) {
						if (response.status == 200) {
							//mp4 exists, play it
							...
							setTimeout(function() { tryHLSPlayer(name); }, 5000);
